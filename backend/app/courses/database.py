...
def get_exercise(con: Connection, cid: int, sid: int, lid: int, eid: int) -> Dict[str, Any]]:
    with con.cursor() as cur:
        cur.execute("""
            SELECT
...
def get_all_exercises(con: Connection, cid: int, sid: int, lid: int) -> List[Dict[str, Any]]:
    with con.cursor() as cur:
        cur.execute("""
            SELECT
                e.id as id
