...
def create_course(con: Connection, title: str, slug: Optional[str] = None, description: Optional[str] = None) -> int:
    print(f"Creating course")
    with con.cursor() as cur:
        cur.execute(
            "INSERT INTO courses(title, slug, description, status) VALUES (%s, %s, %s, 0) RETURNING id",
...
def create_lesson(con: Connection, course_id: int, section_id: int, title: str, description: str, position: int, body_md: Optional[str] = None):
    ...
    print(f"Successfully created lesson")

def get_all_courses(con: Connection) -> List[Dict[str, Any]]: 
